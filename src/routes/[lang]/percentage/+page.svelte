	...
	const guideSections = [
		{
			heading: {
				...
				ja: '50%の損失を取り戻すには100%の利益が必要な理由',
				zh: '为什么 50% 的亏损需要 100% 的收益才能回本'
			},
			body: {
...
실무적 완화법 — (1) 분산은 수익을 비례적으로 줄이지 않으면서 포트폴리오 표준편차를 줄입니다 — Markowitz 평균-분산 최적화(1990 노벨상)가 이를 공식화. (2) 리밸런싱은 손실 후 매수, 이익 후 매도를 강제해 기계적으로 비대칭을 수확. (3) 시간 지평이 중요한 이유는 더 긴 창이 회복을 허용하기 때문 — Vanguard 연구에 따르면 1928년 이후 60/40 포트폴리오의 최악 30년 롤링 기간조차 양의 실질 수익률을 기록. 비대칭 회복 수학은 "buy and hold"가 보유 기간이 길수록 더 잘 작동하는 이유입니다. 교육용일 뿐 — 재무 자문이 아닙니다.`,
				ja: `パーセント計算の中で最も直感に反するのが非対称回復です。損失と利益は、多くの人が期待するようには相殺されません。100ドルの株が50%下落して50ドルになった場合、100ドルに戻るには50%ではなく100%の上昇が必要です。これは、上昇率が元の価格ではなく、小さくなった新しい基準値に対して計算されるためです。L%の損失後に必要な回復率の一般式は次のとおりです: gain_required = L / (100 − L) × 100。50%の損失を代入すると: 50 / (100 − 50) × 100 = 100%。30%の損失では: 30 / 70 × 100 = 42.9%。80%の損失では: 80 / 20 × 100 = 400%。この関係は極端な値になるほど激しく非線形になります。

現実世界での影響は甚大です。S&P 500は2007〜2009年の金融危機で高値から底値まで約56%下落しました(終値ベースで2007年10月から2009年3月まで)。以前のピークに戻るには約127%の上昇が必要で、実際に回復したのは2013年3月 — 約4年に及ぶ回復期間でした。ナスダックは2000年3月から2002年10月のドットコム崩壊で約78%下落し、回復には約355%の上昇が必要でした。その回復には15年かかり、2015年4月にようやく完了しました。ビットコインの2018年の暴落は高値から底値まで約84%で、元に戻るには525%の上昇が必要でした — 実際に回復しましたが、およそ3年かかりました。

この非対称性は、人々が本能的に想定する以上にリスク管理が重要であることを意味します。年間標準偏差20%のポートフォリオ(全株式ポートフォリオの典型)は、約16%の年に20%以上の損失を経験します(ほぼ正規分布を仮定 — 実際の市場は左側の裾がより厚くなっています)。そのようなドローダウンのたびに、回復するだけで25%以上のその後の上昇が必要になります。これが、同じ算術平均でも「なめらかな」リターンが「でこぼこの」リターンより速く複利で増える数学的な理由です。また、トレーダーのポジションサイジングが単一ポジションの損失をポートフォリオの1〜2%に制限するのが一般的な理由でもあります。1回の悪い取引でポートフォリオが50%のドローダウンを被れば、損益分岐点に戻るだけで生き残った資本を2倍にする必要がありますが、2%のドローダウンなら約2.04%の回復で済みます。

実務的な緩和策: (1) 分散投資はリターンを比例的に減らすことなくポートフォリオの標準偏差を下げます — Markowitzの平均分散最適化(1990年ノーベル賞)がこれを定式化しています。(2) リバランスは損失後の買いと利益後の売りを強制し、この非対称性を機械的に収穫します。(3) 時間軸が重要なのは、より長い期間が回復を可能にするからです — Vanguardの調査によれば、1928年以降の60/40ポートフォリオにおける最悪の30年ローリング期間でさえ、プラスの実質リターンを生み出しました。非対称回復の数学こそが、「buy and hold」が保有期間が長いほどうまく機能する理由です。教育目的のみであり、投資助言ではありません。`,
				zh: `百分比数学中最违反直觉的一点是非对称回复：亏损和收益并不会像大多数人预期的那样相互抵消。如果一只 100 美元的股票下跌 50% 至 50 美元，它必须上涨 100%——而不是 50%——才能回到 100 美元。这是因为涨幅百分比是按新的、更小的基数计算的，而不是按原始价格计算。亏损 L% 后所需回复幅度的通用公式为：gain_required = L / (100 − L) × 100。代入 50% 的亏损：50 / (100 − 50) × 100 = 100%。代入 30% 的亏损：30 / 70 × 100 = 42.9%。代入 80% 的亏损：80 / 20 × 100 = 400%。这种关系在极端情况下呈现出剧烈的非线性。

现实世界的利害关系极大。标普 500 指数在 2007–2009 年金融危机中从峰值到谷底下跌了约 56%（按收盘点位计算，2007 年 10 月至 2009 年 3 月）。要回到之前的峰值需要约 127% 的涨幅，而这实际上直到 2013 年 3 月才实现——大约四年的恢复期。纳斯达克在 2000 年 3 月至 2002 年 10 月的互联网泡沫破裂中下跌了约 78%，需要约 355% 的涨幅才能恢复；这一恢复历时 15 年，最终在 2015 年 4 月完成。比特币 2018 年的崩盘从峰值到谷底约为 84%，需要 525% 的涨幅才能回到原位——它确实做到了，但耗时大约三年。

这种非对称性意味着风险管理比人们本能认为的更加重要。年化标准差为 20% 的投资组合（全股票组合的典型水平）在大约 16% 的年份中会经历 20% 以上的亏损（假设近似正态分布——真实市场的左尾更厚）。每一次这样的回撤都需要随后 25% 以上的涨幅才能恢复。这就是在算术平均相同的情况下，"平滑"的回报比"起伏"的回报复利增长更快的数学原因。这也是交易员的仓位管理通常将单一仓位亏损限制在组合 1–2% 的原因：一笔糟糕的交易造成 50% 的组合回撤，需要把幸存的资本翻倍才能回本，而 2% 的回撤只需约 2.04% 的回复。

实用的缓解措施：(1) 分散投资可以在不成比例降低回报的前提下降低组合标准差——Markowitz 的均值-方差优化（1990 年诺贝尔奖）将这一点形式化。(2) 再平衡强制在亏损后买入、在盈利后卖出，从机制上收割这种非对称性。(3) 时间跨度很重要，因为更长的窗口允许恢复——Vanguard 的研究显示，自 1928 年以来 60/40 组合最差的 30 年滚动期间仍然产生了正的实际回报。非对称回复的数学正是"买入并持有"持有时间越长效果越好的原因。仅供教育参考——并非投资建议。`
			},
			code: {
...
recoveryNeeded(84);  // 525.00% — Bitcoin 2018`
		...
		},
		{
			heading: {
				...
				ja: 'パーセンテージポイント vs パーセント: ニュースの見出しを歪める誤用',
				zh: '百分点 vs 百分比：扭曲新闻标题的常见误用'
			},
			body: {
...
여론조사와 설문도 같은 함정에 걸립니다. "후보 A의 지지율이 47%에서 51%로 상승"은 절대값 4 pp 변화지만 — A의 득표율은 (51 − 47) / 47 = 8.5% 상대 증가. 오차 범위 표현이 이를 더 복잡하게 만듭니다 — ±3 pp 오차 범위 여론조사는 진실값이 6 pp 창 안 어디든 있을 수 있다는 뜻이지 6% 상대 창이 아닙니다. Pew Research Center, Gallup, 그리고 다른 주요 여론조사 기관은 모호성을 피하기 위해 방법론 섹션에서 명시적으로 "퍼센트 포인트"를 사용합니다. 헤드라인에서 모호한 "percent" 언어를 보면 — 기저 숫자를 찾아 재계산하세요. 교육용 콘텐츠일 뿐 — 통계 또는 경제 분석은 자격 있는 분석가와 상담하세요.`,
				ja: `ニュースの見出しが「失業率が1%上昇した」と報じるとき、その意味は本当に曖昧です — そして記者はしばしば、話をより劇的に聞こえさせるフレーミングを選びます。失業率が4%から5%になったのであれば、それは1パーセンテージポイント(pp)の増加ですが、相対的には25%の増加です(1 / 4 = 0.25)。どちらの数字も数学的には正しく、異なる事柄を表しています。米連邦準備制度(FRB)、労働統計局(BLS)、そして真面目な金融メディアが「パーセンテージポイント」や「ベーシスポイント」という用語を使うのは、まさにこのギャップのためです。

ベーシスポイント(bp)はパーセンテージポイントの100分の1です: 1 bp = 0.01 pp = 小数で0.0001。FRBは金利決定をベーシスポイントで発表します — 「25ベーシスポイントの利上げ」はフェデラルファンド金利の目標を0.25パーセンテージポイント引き上げます。2023年2月にFRBが金利を4.50%から4.75%に引き上げたとき、それは絶対値で25 bpすなわち0.25 ppでしたが、相対的には5.6%の増加でした(0.25 / 4.50)。それを「5.6%の利上げ」と呼ぶ報道は、技術的には相対変化を計算していることになりますが、絶対的な金利の動きで考えることに慣れた読者を誤解させてしまいます。

この混同は金融商品では金銭的な損失につながります。「12か月間は導入0% APR、その後は現在24.99%の変動APR」というクレジットカードは、絶対値で24.99 pp高くなります。住宅ローン金利の提示が6.50%から6.75%に変わった住宅所有者は、0.25 pp / 25 bpの上昇を経験しましたが、利息コストは相対的に3.85%増加しました。住宅ローンブローカーは、借り手を安心させたいのか警戒させたいのかに応じて、どちらのフレーミングも使い分けます。絶対変化と相対変化という両方の数字を読むことが、修辞的なフレーミングからあなたを守ってくれます。

世論調査やアンケートも同じ罠にはまります。「候補者Aのリードが47%から51%に拡大した」は絶対値で4 ppの変化ですが、候補者Aの得票率は相対的に(51 − 47) / 47 = 8.5%増加しました。誤差範囲の表現がこれをさらに複雑にします: 誤差±3 ppの世論調査は、真の値が6 ppの範囲内のどこにでもあり得るという意味であって、6%の相対的な範囲ではありません。Pew Research Center、Gallup、その他の主要な世論調査機関は、曖昧さを避けるために方法論のセクションで明示的に「percentage points」という表現を使用しています。見出しで曖昧な「percent」という言葉を見たら、元の数字を探して再計算しましょう。教育目的のコンテンツです — 統計・経済分析については資格のあるアナリストにご相談ください。`,
				zh: `当新闻标题报道"失业率上升了 1%"时，其含义确实是模糊的——而记者往往会选择让报道听起来更具戏剧性的表述方式。如果失业率原为 4%、现为 5%，那是 1 个百分点（pp）的增加，但相对增幅为 25%（1 / 4 = 0.25）。两个数字在数学上都正确；它们描述的是不同的东西。美国联邦储备委员会、劳工统计局（BLS）以及严肃的财经媒体正是因为这一差距才使用"百分点"或"基点"这样的术语。

一个基点（bp）是百分点的百分之一：1 bp = 0.01 pp = 小数形式的 0.0001。美联储以基点为单位公布利率决定——"加息 25 个基点"意味着将联邦基金利率目标上调 0.25 个百分点。2023 年 2 月美联储将利率从 4.50% 上调至 4.75% 时，绝对值上是 25 bp 即 0.25 pp——但相对增幅是 5.6%（0.25 / 4.50）。把它称为"加息 5.6%"的新闻报道在技术上确实是在计算相对变化，但会误导习惯以绝对利率变动思考的读者。

在金融产品中，这种混淆会造成金钱损失。一张"前 12 个月 0% 介绍性 APR，之后为目前 24.99% 的浮动 APR"的信用卡，在绝对值上高出 24.99 pp。房贷利率报价从 6.50% 变为 6.75% 的房主，经历了 0.25 pp / 25 bp 的上升，但其利息成本相对增加了 3.85%。房贷经纪人会根据想要安抚还是惊吓借款人来选择使用哪种表述。同时读取两个数字——绝对变化和相对变化——能保护你免受修辞包装的影响。

民意调查和问卷也会掉入同样的陷阱。"候选人 A 的领先优势从 47% 升至 51%"是 4 pp 的绝对变化，但候选人 A 的得票份额相对增长了 (51 − 47) / 47 = 8.5%。误差范围的表述让这更加复杂：误差范围为 ±3 pp 的民调意味着真实值可能落在 6 pp 区间内的任何位置，而不是 6% 的相对区间。Pew Research Center、Gallup 以及其他主要民调机构在方法论部分明确使用"percentage points"以避免歧义。当你在标题中看到含糊的"percent"表述时，去查找底层数字并重新计算。仅为教育内容——统计或经济分析请咨询合格的分析师。`
			},
			code: {
...
const decimal = bp / 10000; // 0.0025`
		...
		},
		{
			heading: {
				...
				ja: '複利のパーセント: なぜ10% + 10%は20%にならないのか',
				zh: '百分比的复利叠加：为什么 10% + 10% 不等于 20%'
			},
			body: {
...
인플레이션도 똑같이 작동합니다. CPI는 2020년 2.0%, 2021년 4.7%, 2022년 8.0%, 2023년 4.1%, 2024년 약 2.9% 상승(BLS 데이터). 단순 합 — 21.7%. 복리 현실 — 1.020 × 1.047 × 1.080 × 1.041 × 1.029 − 1 = 23.6%. 그 1.9 pp 격차는 — 2019년 $1,000 가격표가 2024년에 동일하게 매치되려면 실제 $1,236이 필요하다는 뜻 — 다년 계약 협상이나 은퇴 지출 계산에 알 가치가 있는 숫자. 체인이 길수록 격차가 벌어집니다. 30년 계획에서는 복리 조정이 많은 "엄지손가락 법칙" 단순화를 압도할 만큼 큽니다. 교육용일 뿐 — 구체적 세금과 인플레이션 계산은 CPA 또는 재무 자문가와 상담하세요.`,
				ja: `2つのパーセント変化を積み重ねると、素朴な算術は破綻します。10%の増加の後に10%の減少があっても出発点には戻らず、10%の増加を2回重ねても20%にはなりません。正しい数学は乗法的です: 連鎖するパーセント変化は、(1 + r)の係数を足すのではなく掛け合わせることで合成されます。したがって、10%上昇してから10%下落すると = 1.10 × 0.90 = 0.99、正味1%の損失です。そして10%上昇を2回重ねると = 1.10 × 1.10 = 1.21、正味21%の利益 — この余分な1ポイントこそが、単純計算と複利計算を分ける複利効果です。

この非対称性は時間の経過とともに劇的に複合していきます。50%の上昇の後に50%の下落 = 1.50 × 0.50 = 0.75、正味25%の損失です。下落してから上昇する場合(1.50 × 0.50と0.50 × 1.50)も同じ答えになります。乗法には交換法則が成り立つからです — しかし、それがあなたを救うわけではありません。+50%と−50%の算術平均は0%ですが、実際の結果は−25%です。算術平均と幾何平均のこのギャップは「ボラティリティドラッグ」または「バリアンスドラッグ」と呼ばれ、小さなリターンではおよそσ²/2に等しくなります(σは標準偏差)。平均8%で標準偏差20%のファンドの複利年間成長率はおよそ6%です — 消えた2%がバリアンスドラッグです。

消費者が日常的に数え間違える現実版が、重ね掛けの割引です。「30%オフ、さらにレジで追加20%オフ」は50%オフのように聞こえますが、実際には 1 − (0.70 × 0.80) = 1 − 0.56 = 44%オフです。小売業者はこれを知っていて、見出しの数字を利用しています。逆に、重ね掛けの税も同じように機能します: 7%の売上税に、州税適用後の金額に対して計算される5%の地方追加税を加えると、1.07 × 1.05 − 1 = 12.35%となり、12%ではありません。The Wall Street Journalなどの金融メディアは、政治家やマーケターがこれらを不注意に使うとき、こうした複利の誤りを日常的に指摘しています。

インフレも同じです。CPIは2020年に2.0%、2021年に4.7%、2022年に8.0%、2023年に4.1%、2024年に約2.9%上昇しました(BLSデータ)。素朴な合計: 21.7%。複利の現実: 1.020 × 1.047 × 1.080 × 1.041 × 1.029 − 1 = 23.6%。この1.9 ppのギャップは、2019年に1,000ドルだった値札が2024年に同等であるためには実際には1,236ドル必要という意味です — 複数年契約の交渉や退職後の支出を見積もるなら知っておく価値のある数字です。連鎖が長いほどギャップは広がります。30年の計画では、複利による調整は多くの「経験則」的な単純化を圧倒するほど大きくなります。教育目的のみです — 具体的な税金とインフレの計算はCPAまたはファイナンシャルアドバイザーにご相談ください。`,
				zh: `把两次百分比变化叠加起来，朴素的算术就会失灵：先上涨 10% 再下跌 10% 并不会回到起点，连续两次上涨 10% 也不等于 20%。正确的数学是乘法：连锁的百分比变化通过将各自的 (1 + r) 因子相乘来复合，而不是相加。因此先涨 10% 再跌 10% = 1.10 × 0.90 = 0.99，净亏 1%。连涨两次 10% = 1.10 × 1.10 = 1.21，净赚 21%——多出的这 1 个点就是区分单利数学与复利数学的复利效应。

这种非对称性会随时间急剧放大。先涨 50% 再跌 50% = 1.50 × 0.50 = 0.75，净亏 25%。先跌后涨（1.50 × 0.50 与 0.50 × 1.50）给出相同的答案，因为乘法满足交换律——但这并不能救你。+50% 和 −50% 的算术平均是 0%，但实际结果是 −25%。算术平均与几何平均之间的这一差距被称为"波动率拖累"或"方差拖累"，对于较小的收益率，它大约等于 σ²/2，其中 σ 是标准差。一只平均收益 8%、标准差 20% 的基金，其复合年增长率大约只有 6%——消失的那 2% 就是方差拖累。

叠加折扣是消费者经常算错的现实版本。"先减 30%，结账时再额外减 20%"听起来像是减 50%，但实际上是 1 − (0.70 × 0.80) = 1 − 0.56 = 减 44%。零售商深知这一点并利用这样的宣传标语。反过来，叠加的税也以同样的方式运作：7% 的销售税，加上按加计州税后金额计算的 5% 地方附加税，得到 1.07 × 1.05 − 1 = 12.35%，而不是 12%。当政客或营销人员随意使用这些数字时，《华尔街日报》等财经媒体经常指出这类复利错误。

通货膨胀也是如此。CPI 在 2020 年上涨 2.0%，2021 年上涨 4.7%，2022 年上涨 8.0%，2023 年上涨 4.1%，2024 年上涨约 2.9%（BLS 数据）。朴素求和：21.7%。复利现实：1.020 × 1.047 × 1.080 × 1.041 × 1.029 − 1 = 23.6%。这 1.9 pp 的差距意味着 2019 年标价 1,000 美元的商品到 2024 年实际需要 1,236 美元才能持平——如果你在谈判多年期合同或估算退休支出，这个数字值得了解。链条越长，差距越大。对于 30 年的规划而言，复利调整之大足以令许多"经验法则"式的简化相形见绌。仅供教育参考——具体的税务和通胀计算请咨询注册会计师（CPA）或理财顾问。`
			},
			code: {
...
varianceDrag(0.20);  // 0.02 — 2% drag from 20% volatility`
	...
	const toolContent = {
		...
		faq: {
			...
			ja: [
				{ q: '増減率はどのように計算されますか?', a: '公式は ((新しい値 − 元の値) / 元の値) × 100 です。結果が正なら増加、負なら減少を表します。' },
				{ q: '結果がNaNやInfinityになるのはなぜですか?', a: 'おそらくゼロで割っています(例: 「全体」や「From」の値が0)。分母には0以外の値を入力してください。' },
				{ q: '税込み計算はできますか?', a: 'はい。「YのX%」に税率を入れて税額を求め、元の価格に足してください。' },
				{ q: 'マイナスの割引率は値上げを意味しますか?', a: 'はい。割引率が負の場合、最終価格は定価より高くなります。' }
			],
			zh: [
				{ q: '增减率是如何计算的？', a: '公式为 ((新值 − 旧值) / 旧值) × 100。结果为正表示增加，为负表示减少。' },
				{ q: '为什么结果显示 NaN 或 Infinity？', a: '很可能是除以了零（例如"总数"或"From"值为 0）。请输入非零的分母。' },
				{ q: '可以计算含税价格吗？', a: '可以。用"Y 的 X%"配合税率求出税额，再加到原价上即可。' },
				{ q: '负的折扣率表示加价吗？', a: '是的。折扣率为负时，最终价格会高于原价。' }
			]
		},
		related: [
